...
export default class ForecastProductListV2 extends LightningElement {
    isLoading = false;
    ...
    get detailsBtnLabel() {
        return this.disabled ? 'View Details' : 'Edit / Details';
    }
    ...
    @api getForecast() {
        if (this.searchByProduct) {
            this.getForecastByProduct();
        } else {
            this.getForecastByAccount();
        }
    }

    getForecastByAccount() {
        getForecast({
    ...
    getForecastByProduct() {
        getForecastByProduct({
            ...
            .catch((error) => {
                showError('An error occurred while processing your request. Please reach out to system admin to resolve this issue.');
                console.error('Forecast By Product Error:', error);
            })
            .finally(() => {
                this.isLoading = false;
            });
    ...
    setProducts(products) {
        return products.map(product => {
            ...
            // Convert forecastTotalMap into an array
            ...
            // Map forecastTotalArr to forecastRevenueArr
            const forecastRevenueArr = forecastTotalArr.map(item => {
                // Remove commas and ensure proper numeric parsing
